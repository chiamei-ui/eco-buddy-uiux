...
> ⚠️ **英文標籤準據（2026-08-03 稽核補註）**：本表英文標籤為 RFP v1.4 **§4.1 主表**原文。RFP v1.4 §4.1 **附表 A／附表 B** 及命名手冊 v1.2（並原樣承接至事件介面版 v1.3）對其中 18 項型態（#02、#03、#04、#08、#09、#10、#11、#12、#13、#14、#16、#19、#21、#22、#23、#24、#25、#26）使用另一套英文標籤（如 #09 Sparkling Sprite、#24 Bouncy Daifuku）。甲方已裁定**以 §4.1 主表為準**（理由：主表為型態規格正表，且 §9.1 FX 表備註與附件驗收檢核表均引用主表版本）；附表與命名手冊之 label 欄修正將依合約第 11(1) 條另案書面修訂，`element_naming`（`state_*`）不受影響。修訂生效前，[NAMING.md](NAMING.md) §四仍保留附表 B 原文字。
...
> **#34 內部產品決議（非 RFP 原文，2026-08-03 稽核補註）**：PM #34/#35 決議「連續天數採個人化計算（不綁自然日）、補簽計入連續達標」。該決議**未經合約第 11(1) 條書面修訂**，RFP v1.4 §4.2 現行 binding 文字為「連續 7 自然日每日登入」，本表已回復原文。驗收一律以 RFP 原文為準；如確定採用 PM 決議，須先完成 RFP 修訂與雙方書面確認。

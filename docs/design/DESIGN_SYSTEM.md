...
4. **Buddy 是主角，但不強制每頁都出現**：若 Buddy 視覺會讓畫面複雜、壓縮重要資訊，可改用中性 UI；以各頁 `UI_SPEC.md` 與 hi-fi prototype 為準。
5. 鼓勵 ＞ 命令；陪伴 ＞ 提醒
6. 情緒 ＞ 資訊（少三個數據，多一個表情）
...
### 絕對禁區（UI 文字）
- 「回收、投遞、掃描、任務、KPI」— 原則上不得出現在用戶可見處；P1 主行動「掃描條碼」與 P8 FAQ 為精準理解例外。
...
| 回收 / 掃碼 | 帶禮物回家 / 給 Buddy | 用戶可見文字（按鈕、標題、提示）；P1 主行動「掃描條碼」與 P8 FAQ 可為精準理解保留原詞 |
| 點數 | 點數（保留）| ECOCO 點數維持原名，不改為「愛心」|
| 心情值 | 心情 | 屬性條與對話文案 |
| 角色 / 寵物 | Buddy | 對外文案一律稱呼「Buddy」|
| 圖鑑 | 夥伴日誌 | — |
| 完成（任務領取） | 可領取 | CTA 按鈕文案 |
| 屬性 | 狀態 | 對外文案 |
...
- **底色**：預設奶油膚（`#FAE0B8`）；hi-fi prototype base screen 白底已通過設計審核，工程實作依頁面規格與 prototype。
- **避免使用**：冷色系工具感設計、多種主色競爭；必要少量漸層可保留，主 CTA 仍維持純色。
...
### 絕對禁止（UI 視覺）
- **不要濫用漸層色** — 主按鈕一律純色；prototype 中已通過的必要少量漸層可保留。
- **不要把白底當成冷冰冰工具頁** — hi-fi prototype base screen 白底可用，但卡片、留白、圓角與陰影需維持 ECO Buddy 語氣。
- **不要為了塞 Buddy 犧牲資訊層級** — Buddy 是核心角色；但若每頁放 Buddy 會壓縮重要資訊，可不放，依 `UI_SPEC.md` / prototype 判斷。
...
### 絕對禁止（UI 文字）
- 「回收、投遞、掃描、任務、KPI」原則上不得出現在用戶可見文字；P1「掃描條碼」與 P8 FAQ 可為精準理解保留原詞
...
預設頁面底色 #FAE0B8（奶油膚），hi-fi prototype base screen 可用白底；主色 #FF5000（橘），次色 #060E9F（藍），字型 Noto Sans TC。
卡片 bg-white rounded-2xl shadow-sm，按鈕 CTA rounded-full。
Buddy 視覺不強制每頁出現；若會壓縮重要資訊，以頁面資訊層級優先。
所有面向用戶的文字禁用「回收、投遞、掃描、任務」，改用 Buddy 世界語言；P1「掃描條碼」與 P8 FAQ 為精準理解例外；「ECOCO 點數／點數」維持原名。
不要新增不必要漸層；prototype 已通過的少量漸層可保留。
```

### 建立新頁面
```
使用 DESIGN_SYSTEM.md 建立 [頁面描述]。
- 頁面背景: 預設 bg-[#FAE0B8]（奶油膚）；hi-fi prototype 指定白底時可用白底
